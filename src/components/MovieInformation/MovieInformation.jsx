...
function MovieInformation() {
    ...
    //Ошибкаdocument.addEventListener('DOMContentLoaded', kinoplayer_run);
    ...
    function getDataKinoplayertop(el) {
        var options = getOptionsKinoplayertop(el),
            p = '';
    ...
    function runKinoplayertop(sel) {
        ...
        var bg = options.bg && options.bg.replace(/[^0-9a-z]/gi, '') ? options.bg.replace(/[^0-9a-z]/gi, '') : '2196f3';
        ...
        if (classname) {
            ...
            kinoplayertop.setAttribute(
                'style',
                'position: relative !important; display: block !important; width: 100% !important; min-width: 240px !important; padding-top: ' +
                    (ratio_arr.has(ratio) ? ratio_arr.get(ratio) : '56.25') +
                    '% !important; padding-bottom: 0px !important; overflow: hidden !important; background: #' +
                    bg +
                    ' !important;',
            );
        }
        ...
        l.setAttribute(
            'style',
            'position:absolute;left:0;top:0;width:100%;height:100%;background:#' +
                bg +
        ...
        b.setAttribute(
            'style',
            'display: none;z-index:9999;position:absolute;left:0;top:0;width:100%;height:100%;background-color: #fc1100;background-image:linear-gradient( 20deg, #fa1100 0%, #' +
                bg +
                ' 100%);',
        );
        ...
        e.setAttribute(
            'style',
            'cursor:pointer;color:#721c24;background-color:#f8d7da;border-color:#f5c6cb;font-size:1.9vw;position:absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);padding: 0.75rem 1.25rem;margin-bottom: 1rem;border: 1px solid transparent;border-radius: 0.25rem;',
        );
        e.innerHTML = 'Ошибка загрузки плеера! Повторить?';
        e.onclick = function () {
            getResponseKinoplayertop(false, kinoplayertop);
        };
        b.appendChild(e);
        ...
        if (!classname) {
            i.setAttribute(
                'style',
                'position: absolute !important; top: 0 !important; bottom: 0 !important; left: 0 !important; right: 0!important; width: 100% !important; height: 100% !important; border: 0 !important; background: #000000;',
            );
        }
        i.onload = function () {
    ...
    return (
        <Grid container className="movieinfo" alignItems="space-around">
            <Grid item sm={12} lg={4} style={{ display: 'flex', alignItems: 'center', direction: 'column' }}>
